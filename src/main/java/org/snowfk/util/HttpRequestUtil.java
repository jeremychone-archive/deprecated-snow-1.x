...
public class HttpRequestUtil {
    ...
    /**
     * @param pathInfo
     * @return The last pageName of a URI.
     */
    static public String getPageName(String pathInfo) {
        String pageName = pathInfo;

        // remove all the char before the '/'
        int lastSlashIdx = pathInfo.lastIndexOf("/");
        if (lastSlashIdx != -1) {
            pageName = pathInfo.substring(lastSlashIdx + 1);
        }
    ...
    /**
     * This method works around a Tomcat bug which return a pathInfo null when
     * the servlet mapping is "/". This method will return the expected
     * pathInfo.
     * 
     * @param req
     * @return PathInfo of the request (full path - domain - contextPath)
     */
    public static String getPathInfo(HttpServletRequest req) {
        String pathInfo = req.getPathInfo();

        // if the pathInfo is null, then try to get it from the attribute (which
        // would have been set by the rooter
        // servlet)
        if (pathInfo == null) {
            pathInfo = (String) req.getAttribute("pathInfo");
        }

        // if the pathInfo is still null, then try to compute it
        if (pathInfo == null) {
