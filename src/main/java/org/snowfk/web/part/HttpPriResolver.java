...
public class HttpPriResolver {
    ...
    private static String getPagePriPath(String pathInfo, String modPrefix) {
        ...
        String[] pathInfoAndExt = FileUtil.getFileNameAndExtension(pathInfo);

        // if it ends with .ftl, then remove it
        if (pathInfoAndExt[1].equalsIgnoreCase(".ftl")) {
            pathInfoAndExt[1] = "";
        }

        final String TIME_STAMP_DELIM = "__";
        // if the pathInfoName end with TIME_STAMP_DELIM then we have a timestamps to be removed
        // so, a path like "/css/mycss__someversionid__.css" will map to "/css/mycss.css"
        if (pathInfoAndExt[0].endsWith(TIME_STAMP_DELIM)) {
            int idx = pathInfoAndExt[0].substring(0, pathInfoAndExt[0].length() - TIME_STAMP_DELIM.length()).lastIndexOf(TIME_STAMP_DELIM);
            if (idx > -1) {
                pathInfoAndExt[0] = pathInfoAndExt[0].substring(0, idx);
            }
        }

        // rebuild the pathInfo
        pathInfo = new StringBuilder(pathInfoAndExt[0]).append(pathInfoAndExt[1]).toString();
        ...
        if (lastSlashIdx != -1) {
            ...
            //do not remove the '_' for the web bundle
            if (!pageName.startsWith(WEB_BUNDLE_ALL_PREFIX) && pageName.startsWith("_")) {
