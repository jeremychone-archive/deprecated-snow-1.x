...
public class WebApplicationLoader {
    ...
    public File getWebAppFolder() {
        ...
        // Linux hack (somehow on Linux when contextPath empty, the webAppFolder.getName() return ".", so, if this
    ...
    public WebApplicationLoader load() throws Exception {
        ...
        if (!propertiesFile.exists()) {
            throw new FileNotFoundException("File application.properties not found in directory: " + sfkFolder.getAbsolutePath());
        }
        Properties appProperties = new Properties();
        appProperties.load(new FileReader(propertiesFile));

        //second load the config.properties. If we have a config.properties.location load it from there.
        ...
        if (jndiConfigLookup != null) {
            ...
            try {
                ...
                SnowRuntimeException se = new SnowRuntimeException(Alert.JNDI_CONFIG_PROPERTIES_JNDI_VALUE_NOT_FOUND, e,
                                        "snow.jndiConfigLocation", configLocation);                
                logger.error(se.getMessage());
                throw se;
            }

            //TODO: Should we close the InputStream or does the Properties.load closes ti?
                ...
                //otherwise, assume a URI of a file
                    ...
                    if (!configFile.exists()) {
                        SnowRuntimeException se = new SnowRuntimeException(Alert.JNDI_CONFIG_PROPERTIES_FILE_NOT_FOUND,
                                                "snow.jndiConfigLocation", configLocation, "configFilePath", configFile
                                                                        .getAbsoluteFile());
                        logger.error(se.getMessage());
                        throw se;
                    }
        ...
        //System.out.println("Loading config from JNDI location: " + jndiConfigLocation);
        ...
        // --------- /Loading the config.properties --------- //
        //if a ServletContext, then look if there is a WebApp instance application.appname.properties
