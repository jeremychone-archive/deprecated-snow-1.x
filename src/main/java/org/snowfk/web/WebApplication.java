...
public class WebApplication {
    ...
    //_this is a quick hack for PartResolver. There might be a better way, but not sure. 
    ...
    public synchronized void init() {
        if (!initialized) {
            ...
            //FIXME: Need to support multi-interceptor. Right now, just support one (take the first one)
            ...
            //NOTE: This has to be done after the webModule.init() since it will need to get the TemplateModelProxy
    ...
    public Part getPart(String pagePri, String framePri) {
        ...
        if (framePri != null) {
            ...
            //if it is a content part and it has a '[]' in the path, then, explode the file path
        ...
        //check if it is a .json part, if yes, remove the .json from the extension
    ...
    public void processJsonPart(Part part, RequestContext rc) throws Exception {
        ...
        //if it is the _actionResponse.json, the, just get the data from WebActionResponse
    ...
    public WebActionResponse processWebAction(String webModuleName, String webActionName, RequestContext rc) throws Exception {
        ...
        Object result = webActionRef.invokeWebAction(rc);
    ...
    public void proccessWebModels(String webModuleName, Map m, RequestContext rc) throws Exception {
        ...
        WebModelRef rootMmr = webModule.getWebModelRef("/");
        if (rootMmr != null) {
            rootMmr.invokeWebModel(m, rc);
        }
        ...
        for (int i = 0; i < priPaths.length; i++) {
            ...
            WebModelRef mmr = webModule.getWebModelRef(path);
            if (mmr != null) {
                mmr.invokeWebModel(m, rc);
            }
        }
